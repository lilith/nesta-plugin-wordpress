...
# First - Install the 'nokogiri' and 'hpricot' gems. You can edit your Gemfile to only require these on your dev machine.
# group :development do
#	  gem 'hpricot'
#	  gem 'nokogiri'
# end

#1. Synchronize your blog comments with IntenseDebate. Make sure they show up - check several pages afterwards, you may have to manually run a full import.
...
#5.  URL rewrite (using nesta-plugin-wordpress)
...
#6.  URL redirect from each space-delimited value in 'Alternate Urls' of each page metadata to the actual page path (using nesta-plugin-aliases)
#7.  Configure IntenseDebate to use the page.metadata["Post ID"] for the idcomments_post_id value (if present) (Requires view editing)
...
# - HTML to XML conversion is done with Hpricot followed by Nokogiri. Hpricot has the best element closure detection, Nokogiri enforces proper entity encoding.
...
# - Need to answer to these URLs:
# - /tag/{tag}/ to ??
# - /category/{category}/ to ??
# - /category/{category}/{child-category}/ to ??
# - Use the flags feature instead of status
# - Show html warnings, but use wordpress 
